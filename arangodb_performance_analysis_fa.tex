...
\lr{Persistent} & پرس‌وجوهای عمومی & \lr{O(log n)} \\
\lr{Hash} & جستجوهای تساوی & \lr{O(1)} \\
\lr{Skiplist} & پرس‌وجوهای دامنه‌ای & \lr{O(log n)} \\
\lr{Fulltext} & جستجوی متن & \lr{O(k)} \\
\lr{Geo} & پرس‌وجوهای مکانی & \lr{O(log n)} \\
\lr{TTL} & اسناد منقضی‌شونده & \lr{O(1)} \\ \bottomrule
\end{tabular}
\end{table}
