...
تمام پایگاه‌داده‌ها با تنظیمات توصیه‌شده تولید پیکربندی شدند و تخصیص حافظه به \lr{16} گیگابایت برای هر نمونه نرمال‌سازی شد.

\subsection{سخت‌افزار و محیط}

\textbf{تست‌های تک‌گره:}
\begin{itemize}
    \item پردازنده: \lr{AMD EPYC 7543} (32 هسته @ 2.8 گیگاهرتز)
    \item حافظه رم: \lr{64}\ گیگابایت \lr{DDR4}
    \item ذخیره‌سازی: \lr{1} ترابایت \lr{NVMe SSD}
    \item سیستم عامل: \lr{Ubuntu 22.04 LTS}
\end{itemize}

\textbf{تست‌های خوشه:}
\begin{itemize}
    \item خوشه \lr{3} گره‌ای (همان سخت‌افزار برای هر گره)
    \item اتصال شبکه \lr{10} گیگابیت بر ثانیه
...
\begin{itemize}
    \item \lr{10} میلیون سند کاربر.
    \item میانگین اندازه: \lr{2} کیلوبایت.
    \item اسکیما: {\latin\texttt{\{id, name, email, age, country, registration\_date, preferences\}}}.
    \item مجموعه داده ثانویه: \lr{50} میلیون سند محصول (میانگین \lr{1.5} کیلوبایت).
\end{itemize}

\subsubsection{مجموعه داده گراف}
\begin{itemize}
    \item شبکه اجتماعی: \lr{1} میلیون راس، \lr{10} میلیون یال.
    \item توزیع درجه قانون توان (گراف اجتماعی واقعی).
    \item میانگین درجه: \lr{10}، حداکثر درجه: \lr{5000}.
    \item ویژگی‌های یال: \texttt{\{timestamp, relationship\_type\}}.
\end{itemize}

\subsubsection{مجموعه داده کلید-مقدار}
\begin{itemize}
    \item \lr{20} میلیون جفت کلید-مقدار.
    \item توزیع کلید: زیفیان (\lr{Zipfian}) (تقلید از الگوهای دسترسی دنیای واقعی).
    \item اندازه مقادیر: \lr{100} بایت تا \lr{10} کیلوبایت (توزیع نرمال حدود \lr{1} کیلوبایت).
\end{itemize}
...
\begin{itemize}
    \item \textbf{\lr{G1}}: پیمایش‌های کوتاه (\lr{1-2} گام)، همزمانی بالا.
    \item \textbf{\lr{G2}}: پیمایش‌های متوسط (\lr{3-4} گام)، همزمانی متوسط.
    \item \textbf{\lr{G3}}: پیمایش‌های عمیق (\lr{5-7} گام)، همزمانی کم.
...
هر آزمایش \lr{5} بار تکرار شد. نتایج گزارش می‌دهند:
\begin{itemize}
    \item مقادیر میانه (مقاوم در برابر داده‌های پرت).
    \item بازه‌های اطمینان \lr{95\%}.
...
\begin{itemize}
    \item \lr{MongoDB} حدود \lr{5} تا \lr{13} درصد توان عملیاتی بالاتری برای عملیات پایه \lr{CRUD} دارد، که بیشترین تفاوت در بارهای کاری درج گروهی به دلیل مسیر نوشتن بسیار بهینه شده آن مشاهده می‌شود.
    \item با وجود این، \lr{ArangoDB} تقریباً \lr{85} تا \lr{95} درصد از توان عملیاتی \lr{MongoDB} را در تمام عملیات \lr{CRUD} حفظ می‌کند که نشان می‌دهد سربار قابلیت‌های چندمدلی آن برای بارهای کاری صرفاً سندی نسبتاً کم است.
...
\textbf{تحلیل:}
\begin{itemize}
    \item برای الگوهای تجمیع ساده مانند \emph{\lr{GROUP BY + COUNT}}، \lr{MongoDB} به طور ناچیزی سریع‌تر است (حدود \lr{2-3\%})، که با بهینه‌سازی آن برای مراحل ساده پایپ‌لاین سازگار است.
    \item با پیچیده‌تر شدن پایپ‌لاین‌های تجمیع پایپ‌لاین‌های چندمرحله‌ای، پرس‌وجوهای شبیه به الحاق، تجمیع‌های تو در تو، \lr{ArangoDB} شروع به پیشی گرفتن از \lr{MongoDB} می‌کند، با بهبودهای تاخیر در بازه \lr{8-20\%}.
    \item در تجمیع‌های شبیه به الحاق که الحاق‌های چندمجموعه‌ای را تقریب می‌زنند، بهینه‌ساز پرس‌وجوی \lr{ArangoDB} موثرتر به نظر می‌رسد و حدود \lr{28\%} تاخیر کمتری را نسبت به \lr{MongoDB} تحت تنظیمات یکسان مجموعه داده و ایندکس به دست می‌آورد.
...
\textbf{تفسیر:}
\begin{itemize}
    \item در تمام بارهای کاری \lr{YCSB}، پایگاه‌داده \lr{MongoDB} به طور مداوم با اختلاف حدود \lr{5} تا \lr{10} درصد از \lr{ArangoDB} بهتر عمل می‌کند، که مشاهدات قبلی \lr{CRUD} را در شرایط دسترسی ترکیبی و سنگین-خواندن تایید می‌کند.
...
\lr{1} گام & \lr{2.3} & \lr{1.8} \\
\lr{2} گام & \lr{8.5} & \lr{7.2} \\
\lr{3} گام & \lr{24.1} & \lr{28.3} \\
\lr{4} گام & \lr{68.2} & \lr{82.5} \\
\lr{5} گام & \lr{185.4} & \lr{235.7} \\
\lr{6} گام & \lr{521.3} & \lr{688.9} \\ \bottomrule
\end{tabular}
\end{table}
...
\textbf{مشاهدات کلیدی:}
\begin{itemize}
    \item برای پیمایش‌های کم‌عمق (\lr{1-2} گام)، \lr{Neo4j} حدود \lr{15-20\%} سریع‌تر از \lr{ArangoDB} است که منعکس‌کننده مزایای طرح ذخیره‌سازی گراف بومی و ساختارهای مجاورتی مبتنی بر اشاره‌گر تخصصی آن برای دسترسی به همسایگان نزدیک است.
    \item با افزایش عمق پیمایش به بیش از \lr{3} گام، \lr{ArangoDB} شروع به پیشی گرفتن از \lr{Neo4j} می‌کند و تقریباً \lr{15-24\%} تاخیر کمتری برای پیمایش‌های \lr{3-6} گام به دست می‌آورد. این نشان می‌دهد که استراتژی‌های بهینه‌ساز و هرس کردن \lr{ArangoDB} برای محدود کردن فضای جستجو در پیمایش‌های عمیق‌تر در این مجموعه داده به طور خاص موثر هستند.
...
\textbf{تحلیل:}
\begin{itemize}
    \item \lr{Redis} در عملیات ساده \lr{GET/SET} (2 تا 7 برابر سریع‌تر) به دلیل طراحی در حافظه و حلقه رویداد تک‌رشته‌ای بسیار بهینه شده، غالب است.
...
تاخیر \lr{p95} اندازه‌گیری شده برای این پرس‌وجو در \lr{ArangoDB} برابر \lr{35} میلی‌ثانیه است.

در معماری چندزبانه، منطق معادل نیاز دارد به:
\begin{enumerate}
    \item یک پرس‌وجوی \lr{Cypher} در \lr{Neo4j} برای بازیابی لیست شناسه‌های دوستان برای کاربر هدف (حدود \lr{12} میلی‌ثانیه).
    \item یک پرس‌وجوی تجمیع در \lr{MongoDB} برای بازیابی محصولات خریداری شده توسط آن دوستان و فیلتر کردن بر اساس دسته‌بندی (حدود \lr{18} میلی‌ثانیه).
    \item منطق سمت برنامه برای الحاق نتایج، تجمیع شمارش‌ها و مرتب‌سازی (حدود \lr{8} میلی‌ثانیه).
\end{enumerate}

تاخیر ترکیبی \lr{p95} برای پایپ‌لاین چندزبانه، بدون احتساب سربار شبکه و سریال‌سازی، تقریباً \lr{38} میلی‌ثانیه است؛ با احتساب میانگین سربار بین سرویس‌ها، تاخیر سرتاسری حدود \lr{52} میلی‌ثانیه است.
...
\begin{itemize}
    \item \lr{ArangoDB} کل پرس‌وجوی پیشنهاد چندمدلی را در حدود \lr{35} میلی‌ثانیه (\lr{p95}) اجرا می‌کند، در حالی که رویکرد چندزبانه به حدود \lr{52} میلی‌ثانیه سرتاسری نیاز دارد که نشان‌دهنده تقریباً \lr{33\%} کاهش تاخیر است.
...
\begin{enumerate}
    \item اینفلوئنسرها را بر اساس معیارهای گراف شناسایی می‌کند (مثلاً کاربرانی با بیش از \lr{1000} دنبال‌کننده).
...
\textbf{بحث مقایسه‌ای:}
\begin{itemize}
    \item \lr{ArangoDB} تحلیل نفوذ اجتماعی را در حدود \lr{245} میلی‌ثانیه (\lr{p95}) به عنوان یک پرس‌وجوی واحد تکمیل می‌کند، در حالی که راهکار چندزبانه به حدود \lr{412} میلی‌ثانیه نیاز دارد که منجر به \lr{40-41\%} کاهش تاخیر می‌شود.
...
\lr{1} & \lr{28.4} & \lr{30.2} & \lr{32.1} \\
\lr{3} & \lr{76.2} & \lr{82.5} & \lr{89.4} \\
\lr{5} & \lr{118.3} & \lr{128.7} & \lr{142.8} \\
\lr{7} & \lr{152.1} & \lr{168.4} & \lr{188.2} \\ \bottomrule
\end{tabular}
\end{table}

\textbf{کارایی مقیاس‌پذیری (\lr{5} گره):}
\begin{itemize}
    \item \lr{ArangoDB}: \lr{83.1\%} (\lr{4.17} برابر افزایش سرعت).
    \item \lr{MongoDB}: \lr{85.2\%} (\lr{4.26} برابر افزایش سرعت).
    \item \lr{Cassandra}: \lr{89.0\%} (\lr{4.45} برابر افزایش سرعت).
\end{itemize}
...
\begin{table}[h]
\centering
\caption{گراف هوشمند در مقابل شاردینگ استاندارد (خوشه \lr{3} گره‌ای، میلی‌ثانیه)}
\label{tab:smartgraph}
\begin{tabular}{@{}lrr@{}}
\toprule
پرس‌وجو & استاندارد & گراف هوشمند \\ \midrule
پیمایش \lr{2} گام & \lr{18.5} & \lr{9.2} \\
پیمایش \lr{3} گام & \lr{52.3} & \lr{26.7} \\
تطبیق الگو & \lr{215.4} & \lr{128.3} \\ \bottomrule
\end{tabular}
\end{table}

گراف‌های هوشمند با هم‌مکان کردن داده‌های گراف مرتبط و به حداقل رساندن ارتباطات شبکه در طول پیمایش‌ها، تاخیر را تقریباً \lr{48-50\%} کاهش می‌دهند. این ویژگی به ویژه هنگام استقرار \lr{ArangoDB} به عنوان یک موتور گراف توزیع شده مهم است.

\subsection{بهره‌وری منابع}

\subsubsection{کارایی حافظه}

استفاده از حافظه برای \lr{10} میلیون سند (\lr{10} گیگابایت اندازه منطقی):
...
\begin{itemize}
    \item \lr{MongoDB}: \lr{1،260 ops/core}.
    \item \lr{ArangoDB}: \lr{1،185 ops/core} (\lr{94\%} از \lr{MongoDB}).
    \item \lr{Cassandra}: \lr{1،340 ops/core}.
\end{itemize}
...
\begin{enumerate}
    \item \textbf{عملیات سند:} \lr{ArangoDB} به \lr{85-95\%} از توان عملیاتی پایگاه‌داده‌های تخصصی برای بارهای کاری سند دست می‌یابد، با برخی پرس‌وجوهای پیچیده که عملکرد بهتری نسبت به گزینه‌های تخصصی دارند.
    \item \textbf{پیمایش‌های گراف:} رقابتی با \lr{Neo4j} برای پرس‌وجوهای کم‌عمق، و \lr{15-24\%} سریع‌تر برای پیمایش‌های عمیق‌تر (\lr{3+} گام) در مجموعه داده و پیکربندی تست شده.
    ...
    \item \textbf{پرس‌وجوهای چندمدلی:} \lr{30-40\%} سریع‌تر از رویکردهای چندزبانه به دلیل حذف سربار هماهنگی و بهینه‌سازی یکپارچه.
    \item \textbf{مقیاس‌پذیری:} مقیاس‌پذیری افقی خوب (حدود \lr{83\%} کارایی در \lr{5} گره)، با گراف‌های هوشمند که عملکرد گراف توزیع شده را به طور چشمگیری بهبود می‌بخشند.
...
برای برنامه‌هایی که به طور انحصاری الگوریتم‌های پیچیده گراف (\lr{PageRank}، تشخیص جامعه و غیره) را اجرا می‌کنند، کتابخانه الگوریتم‌های تخصصی \lr{Neo4j} و ذخیره‌سازی گراف بومی ممکن است مزایایی را ارائه دهند، به ویژه برای گراف‌های بسیار بزرگ (بیش از \lr{50} میلیون راس).
...
\textbf{هزینه‌ها:}
\begin{itemize}
    \item \lr{5-15\%} کاهش توان عملیاتی برای عملیات ساده سند در مقایسه با \lr{MongoDB}.
    \item \lr{15-20\%} پیمایش‌های گراف کم‌عمق کندتر در مقایسه با \lr{Neo4j}.
    \item عملیات کلید-مقدار به طور قابل توجهی کندتر در مقایسه با \lr{Redis} در حافظه.
\end{itemize}

\textbf{مزایا:}
\begin{itemize}
    \item \lr{30-40\%} پرس‌وجوهای چندمدلی سریع‌تر در مقایسه با معماری‌های چندزبانه.
...
اگر بیش از \lr{95\%} پرس‌وجوها از یک مدل داده واحد استفاده کنند، مزایای عملکردی پایگاه‌داده‌های تخصصی بر مزایای چندمدلی می‌چربد. بهبود \lr{5-15\%} توان عملیاتی ممکن است تاثیر قابل توجهی بر هزینه‌های زیرساخت در مقیاس بزرگ داشته باشد.
...
\begin{itemize}
    ...
    \item ذخیره‌سازی گراف بومی \lr{Neo4j} برای گراف‌های با بیش از \lr{100} میلیون راس ضروری است.
...
\begin{enumerate}
    \item \textbf{عملکرد تخصصی رقابتی:} \lr{ArangoDB} به \lr{85-95\%} از توان عملیاتی پایگاه‌داده‌های تخصصی برای بارهای کاری سند و گراف دست می‌یابد، با برخی پرس‌وجوهای پیچیده که عملکرد بهتری نسبت به گزینه‌های تخصصی دارند.
    
    \item \textbf{مزیت چندمدلی:} پرس‌وجوهایی که چندین مدل داده را ترکیب می‌کنند، \lr{30-40\%} سریع‌تر در \lr{ArangoDB} نسبت به پیاده‌سازی‌های چندزبانه معادل اجرا می‌شوند، که ناشی از حذف سربار هماهنگی و بهینه‌سازی یکپارچه است.
    
    \item \textbf{مقیاس‌پذیری:} \lr{ArangoDB} مقیاس‌پذیری افقی خوبی نشان می‌دهد (حدود \lr{83\%} کارایی در \lr{5} گره)، با گراف‌های هوشمند که عملکرد پرس‌وجوی گراف توزیع شده را به طور قابل توجهی بهبود می‌بخشند.
...
\begin{itemize}
    \item بیش از \lr{95\%} بار کاری از یک مدل داده واحد استفاده می‌کند.
...
بزرگترین تست‌های ما از گراف‌هایی با \lr{50} میلیون راس استفاده کردند. ارزیابی در مقیاس‌های بزرگتر (میلیاردها موجودیت) نشان خواهد داد که آیا ویژگی‌های عملکرد به طور بنیادی تغییر می‌کنند و آیا گراف‌های هوشمند \lr{ArangoDB} مزایای خود را حفظ می‌کنند.
...
پارادایم پایگاه‌داده چندمدلی، که با نمونه \lr{ArangoDB} نشان داده شده است، یک مصالحه عملی بین عملکرد تخصصی و سادگی عملیاتی را نشان می‌دهد. نتایج ما نشان می‌دهد که این مصالحه اغلب مطلوب است: جریمه عملکرد برای بارهای کاری تخصصی متوسط است (\lr{5-15\%})، در حالی که مزایا برای سناریوهای چندمدلی قابل توجه است (\lr{30-40\%} بهبود).
...
\begin{latin}
\noindent
R.~Gunawan, A.~Rahmatulloh, and I.~Darmawan, 
\emph{Performance Evaluation of Query Response Time in The Document Stored NoSQL Database}, 
in 2019 IEEE 16th International Conference on Quality in Research (QIR): 
